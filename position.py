...
position_spec = [
    ...
    ("enpas", nb.uint8),
    ("castle", nb.uint8),
    ("halfmove_clock", nb.uint8), # halfmove_clock 속성 추가
    ("fullmove_number", nb.uint16), # fullmove_number 속성 추가
    ("repetition_count", nb.uint8), # 반복 횟수 속성 추가
    ("hash_key", nb.uint64),
]
...
def position_to_fen(pos):
            ...
            for side in range(2):
                if get_bit(pos.occupancy[side], sq):
                    ...
                    for piece_type in range(6):
                        if get_bit(pos.pieces[side][piece_type], sq):
                            if side == white:
                                board_fen += piece_to_letter[white][piece_type]
                            else:
                                board_fen += piece_to_letter[black][piece_type]
                            piece_found = True
                            break
                    break
    ...
    # 2. 현재 플레이어
    fen_parts.append("w" if pos.side == white else "b")
    ...
    if not castle_fen:
        castle_fen = "-"
    fen_parts.append(castle_fen)
    
    # 4. 앙파상 타겟
    if pos.enpas == no_sq:
        fen_parts.append("-")
    else:
        fen_parts.append(square_to_coordinates[pos.enpas])
